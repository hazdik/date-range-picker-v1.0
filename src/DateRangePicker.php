...
HTML;
    ...
    public function registerAssets()
    {
        ...
        $js = <<< JS
{$input}.off('change.drp').on('change.drp', function() {
...
JS;
    ...
    protected function initSettings()
    {
        ...
        if (empty($this->containerOptions['class'])) {
            ...
            $this->containerOptions['class'] = 'drp-container' . $css;
        }
        $this->initRange();
        $this->registerAssets();
    }
    ...
    protected function initLocale()
    {
        ...
        $localeSettings += [
            'applyLabel' => Yii::t('drp', 'Apply'),
            'cancelLabel' => Yii::t('drp', 'Cancel'),
            'fromLabel' => Yii::t('drp', 'From'),
            'toLabel' => Yii::t('drp', 'To'),
            'weekLabel' => Yii::t('drp', 'W'),
            'customRangeLabel' => Yii::t('drp', 'Custom Range'),
    ...
    protected function initRange()
    {
        if (isset($dummyValidation)) {
            ...
            $msg = Yii::t('drp', 'Select Date Range');
        }
        if ($this->presetDropdown) {
            $m = 'moment()';
            ...
            $this->pluginOptions['ranges'] = [
                Yii::t('drp', 'Today') => [$beg, $end],
                Yii::t('drp', 'Yesterday') => ["{$beg}.subtract(1,'days')", "{$end}.subtract(1,'days')"],
                Yii::t('drp', 'Last {n} Days', ['n' => 7]) => ["{$beg}.subtract(6, 'days')", $end],
                Yii::t('drp', 'Last {n} Days', ['n' => 30]) => ["{$beg}.subtract(29, 'days')", $end],
                Yii::t('drp', 'This Month') => ["{$m}.startOf('month')", "{$m}.endOf('month')"],
                Yii::t('drp', 'Last Month') => ["{$last}.startOf('month')", "{$last}.endOf('month')"],
            ];
            if (empty($this->value)) {
